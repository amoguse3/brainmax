...
// Stroop -> Rule Switch. L1 ink, L2 opposite, L3+ hidden rule changes triggered by border cue.
...
export function startStroop(){
  ...
  function newQ(){
    ...
    gBody.innerHTML=`<div class="game-hint">SOLID border = ink color, DASH border = opposite color</div><div class="stroop-word" style="color:${ink};border:${border};padding:12px 24px;border-radius:18px">${word}</div><div class="stroop-opts">${opts.map(o=>`<button class="stroop-btn" data-c="${o[0]}">${o[0]}</button>`).join('')}</div>`;
    let answered=false;
    const pick=(c,btn)=>{ if(answered)return; answered=true; rt.push(stopTimer()); document.querySelectorAll('.stroop-btn').forEach(x=>x.style.pointerEvents='none'); if(c===target){ correct++; btn?.classList.add('correct'); vib(20); } else { btn?.classList.add('wrong'); document.querySelector(`.stroop-btn[data-c="${target}"]`)?.classList.add('correct'); vib([30,20,30]); } setTimeout(newQ,480); };
