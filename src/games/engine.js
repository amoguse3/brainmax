import { S, save, setSessionDone, vib, rewardLevelUps } from '../state.js';
...
export function startTimer(ms,onExpire){if(gs.rtimer)clearInterval(gs.rtimer);const t0=Date.now();timerFill.style.width='100%';gs.rExpire=onExpire;gs.rStart=t0;gs.rtimer=setInterval(()=>{const el=Date.now()-t0,pct=Math.max(0,100-el/ms*100);timerFill.style.width=pct+'%';if(el>=ms){clearInterval(gs.rtimer);if(gs.rExpire)gs.rExpire();}},40);}
...
export function finish(icon,acc,avgRtMs,zoneKey,gameKey,extraNote){
 ...
 let milestone=null;const zv=S.zones[zoneKey];if(zv>=100&&prev<100)milestone=zoneMeta[zoneKey].n+' zone MASTERED';else if(zv>=75&&zv-zoneGain<75)milestone=zoneMeta[zoneKey].n+' reached 75';else if(zv>=50&&zv-zoneGain<50)milestone=zoneMeta[zoneKey].n+' badge unlocked';
 const levelUps=rewardLevelUps(gameKey);save();const zm=zoneMeta[zoneKey];
 gBody.innerHTML=`<div class="results show"><div class="results-icon">${icon}</div><div class="results-title">${acc>=70?'Well done':'Good effort'}${isRecord?' · New personal best':''}</div><div class="results-stats"><div class="rst"><div class="rst-v">${acc}%</div><div class="rst-l">Accuracy</div></div><div class="rst"><div class="rst-v">+${cqGain}</div><div class="rst-l">CQ</div></div><div class="rst"><div class="rst-v">${(avgRtMs/1000).toFixed(1)}s</div><div class="rst-l">Speed</div></div></div><div class="trained"><div class="trained-row"><span class="trained-dot"></span><span class="trained-zone">${zm.n}</span><span class="trained-gain">+${zoneGain}</span></div>${levelUps?`<div class="trained-row"><span class="trained-dot"></span><span class="trained-zone">Neuron flow</span><span class="trained-gain">+${(levelUps*.02*3600).toFixed(0)}/h · +${levelUps*40}</span></div>`:''}${extraNote?`<div class="trained-row"><span class="trained-dot"></span><span class="trained-zone">${extraNote}</span></div>`:''}${milestone?`<div class="trained-row"><span class="trained-dot"></span><span class="trained-zone">${milestone}</span></div>`:''}</div><button class="results-cta" id="resCta">Finish session</button></div>`;
 document.getElementById('resCta').addEventListener('click',()=>{completeSession();endGame();});vib(levelUps?[12,35,20,35,28]:(milestone?[15,50,25,50,25]:[15,50,25]));
}
