...
export function makeBrainPts(count) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    points.push({ x, y, z, size: 0.72 + seededNoise(i + 9000) * 0.55 });
  }
  return points;
}
...
export function mountStatBrain() {
  ...
  function draw(now) {
    ...
    const projected = points.map((point) => {
      ...
      return { x: centerX + x * scale * perspective, y: centerY - y * scale * perspective, depth, perspective, size: point.size };
    });
